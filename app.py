...
def run_self_repair():
    ...
    # Memory Sanitization
    # Smart Memory Sanitization
    cutoff_time = time.time() - 5.0 # Remove objects not seen in last 5 seconds
    stale_ids = []
    ...
    if stale_ids:
        ...
        # st.session_state.repair_logs.append(f"[{datetime.now().strftime('%H:%M')}] Sanitized {len(stale_ids)} stale tracks")
...
@st.cache_resource
def load_ocr():
    return easyocr.Reader(['en'], gpu=False) # GPU=False for compatibility

def check_ambulance_text(crop):
    if crop is None or crop.size == 0: return False
    try:
        reader = load_ocr()
        # Fast mode with low detail for speed
...
def is_ambulance_match(crop, tid):
    ...
    # Color-Based Detection (Fast & Reliable)
    # OCR disabled to prevent startup lag - color matching is sufficient
...
track_data = {}
smooth_boxes = {}  # For visual smoothing: {id: (x1, y1, x2, y2)}
...
with st.sidebar:
    ...
    # Sync model selection (Moved after definition)
...
def process_vehicle_logic(tid, cx, cy, t, h_frame):
    ...
    if len(data['history']) >= 2:
        ...
        if dt > 0.001:
            ...
            vector = (dx, dy)
            data['vector'] = vector
            # Refined PPM: 18.5 base for 1080p, scaled to 640p (approx 0.6x) -> 11.5
...
def run_engine():
    try:
        ...
        # Check if video file exists before opening
        ...
        # Optimization: OCR will lazy-load on first valid candidate
        # Removed blocking pre-load to fix startup hang
        ...
        # OPTIMIZED: Reduced resolution for higher FPS (640px width)
        ...
        while cap.isOpened() and st.session_state.active:
            ...
            # ref_kp, ref_des = load_ambulance_ref() # Disabled: Asset missing
            ...
            # Risk scores needed for heatmap, so compute risk first? (Circular dependency in request, simplified here)
            # Use blank scores for first heatmap update or previous frame's scores?
            # Let's compute risk first, passing None for heatmap initially or current heatmap
            # To follow prompt order: heatmap then risk. 
            # Heatmap removed
            ...
            # 2. Trajectories (Upgraded Visualization)
            for tid, points in trajectories.items():
                if len(points) > 5:
                    # Color based on risk/speed?
                    # Default: Cyan/Blue for standard path
            ...
            for obs in obstructions:
                 # Find vehicle box to draw red rect?
                 # blocking_vehicle_id
                 vid = obs['blocking_vehicle_id']
                 # Find in tracked to get box
            ...
            for d in cached_detections:
                ...
                # Apply exponential smoothing to box positions for visual stability
                tid = d['id']
                x1, y1, x2, y2 = d['box']
            ...
            # Cap at 60 FPS and run periodic maintenance
        ...
        # time.sleep(2); st.rerun()  # Disabled auto-rerun to prevent error loops
